...
from src.routers.game import mezclar_figuras
import pytest
...
@patch("src.routers.game.repartir_cartas_figuras")
def test_endpoint_mezclar_figuras_4players(mock_repartir_cartafigura):
    ...
    with patch("src.routers.game.random.shuffle", side_effect=lambda x: x):
